...
-- Pujar un 5% el salari dels managers
UPDATE staff SET salary=salary*1.05 WHERE position='Manager';

SELECT * FROM staff;

-- Promou l'empleat David Ford (SG14) a Manager amb salari 18000
UPDATE staff SET position='Manager', salary=18000 WHERE staff_id='SG14';

SELECT * FROM staff;

SELECT * FROM viewing;

-- Elimina totes les visites que s'hagin realitzzat a la propietat PG4
DELETE FROM viewing WHERE property_id='PG4';

SELECT * FROM viewing;

-- afegir el camp "country" a la taula BRANCH
ALTER TABLE branch ADD COLUMN country text;

\d branch;

-- Esborrar el camp creat
